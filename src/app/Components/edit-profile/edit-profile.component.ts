...
export class EditProfileComponent {
  ...
  initForm(): void {
    ...
    // if (this.action === 'edit') {
      ...
      this.parentImage = this.userData.image;
      this.responsibleGuardianImage = this.userData.responsibleGuardianImage;
      this.nationalIdImage = this.userData.nationalIdImage;

      // Por ejemplo, si tuvieras un @Input() parentData: any;
      // this.profileForm.patchValue(this.parentData);
    // }
  }
  ...
  async onSubmit() {
    ...
    this.profileForm.markAllAsTouched(); // Asegura que los errores se muestren en el HTML

    console.log('Estado completo del formulario:', this.profileForm);
    console.log('Errores del formulario (si existen):', this.profileForm.errors); // Esto suele ser null a nivel de FormGroup
    console.log('¿Formulario Válido?', this.profileForm.valid);

    if (this.profileForm.valid) {
      // if (this.action == 'add') {

      // this.parentService.postParent(this.profileForm.value).subscribe({
      //   next: (response: any) => {
      //     console.log(response, 'respo ,,,,,,,,,,,,,,,,,,,,,,');
  ...
  removeImage(cont: any) {
    if (cont == 1) {
      // this.profileForm.
      this.parentImage = '';
      this.profileForm.get('parentImage')?.setValue(this.parentImage);
    }
    if (cont == 2) {
      this.nationalIdImage = '';
      this.profileForm.get('nationalIdImage')?.setValue(this.nationalIdImage);
    }
    if (cont == 3) {
