...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loginForm = this.fb.group({
      ...
      fcm_token: [''],
      operatingSystem: [operatingSystem, Validators.required],
    ...
    if (login && password) {
      ...
      this.loginService.login(this.loginForm.value).subscribe({
        next: (response: any) => {
          ...
          // this.userData =
          this.getParent(
            JSON.parse(JSON.stringify(response) || '{}')?.userInfo
          );
          // ?.userInfo;
      ...
      this.toastService.presentToast(
        'Usuario o contraseña no tiene información. Por favor complete todos los campos.'
      );
    }
  }
